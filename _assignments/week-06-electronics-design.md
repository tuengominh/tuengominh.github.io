...
<p>This week's content was a bit easier for me to digest since I read about the basic components during the <a href="http://academany.fabcloud.io/fabacademy/2020/labs/barcelona/students/tue-ngo/assignments/week-04-electronics-production.html#theory">4th week</a>. This week, I wanted to learn about the principles of designing a functional citcuit. Allow me to go through a very long note without any images attached, or go straight to the assignments <a href="#assignment">below.</a></p>

<h2 id="theory">Understanding EDA</h2>
<p><a href="https://en.wikipedia.org/wiki/Electronic_design_automation">Electronic design automation (EDA)</a> is a type of software used to design and simulate electronic systems such as ICs and PCBs. An EDA usually must be able to help us create schematics, generate <a href="https://en.wikipedia.org/wiki/Netlist">Netlists</a>, create PCB layouts (including route tracks and <a href="https://en.wikipedia.org/wiki/Footprint_(electronics)">footprints</a>), and generate <a href="https://en.wikipedia.org/wiki/Gerber_format">Gerber</a> files. I was thinking about sticking to the Autodesk family by using their EDA software Eagle. However, there were some reasons I went ahead with KiCad: it is free and open-source, and our instructor Oscar provided us with a very throughout tutorial regarding design steps using KiCad <a href="http://fabacademy.org/2020/labs/barcelona/local/#material/extras/week06/kicad">here.</a></p>
<h5>EDA design flows</h5>
<p>We indeed need to go through a bunch of EDA flows to accomplish the design of an IC or a PCB. Some crucial steps:</p>
<ul>
<li><strong>Schematic capture:</strong> the very first step of the actual design of an electronic circuit. Besides basic drawing and visualization capabilities, schematic editors typically provide the capabilities of hierarchical design by capturing electrical properties and other attributes of components, wires, nets, and pins.</li>
...
<li>Place all components on the same side of the board. Avoid placing components on the edge of the board by making sure that all of the components are at least 1.5 - 2mm away from the edge of the PCB.</li>
...
<p>Now I have the new self-made UPDI component!</p>
<p></p>
<h6>Designing schema with KiCad Eeschema</h6>
<p>I started with sketching my board on <a href="https://kicad-pcb.org/discover/eeschema/">Eeschema</a> schematic editor first. To understand the ATtiny1614 chip and its pins, I refer to the datasheet <a href="http://ww1.microchip.com/downloads/en/DeviceDoc/ATtiny1614-data-sheet-40001995A.pdf">here.</a></p>
<p>The workflow went like:</p>
<ul>
<li>Create new <code>.pro</code> KiCad project and open the <code>.sch</code> file in Eeschema</li>
<li>Choose the <strong>Place component</strong> tool and select the components needed from the Fab library. The components I used: <i>ATtiny1614-SS</i> for the IC, <i>FTDI-SMD-HEADER</i> for the FTDI header, <i>6MM_SWITCH6mm_SWITCH</i> for the button, <i>CAP-UNPOLARIZEDFAB</i> for the capacitor, <i>RES-US1206FAB</i> for the resistors, <i>LEDFAB1206</i> for the LEDs, and my custom <i>UPDI-SMD-HEADER</i> for the UPDI header.</li>
<li>Edit the properties of the components by hovering and press <kbd>E</kbd>: annotations, values(Ω, uF, GREEN), and footprints. In order to calculate the resistance value of the resistors, I also took a look at this <a href="https://datasheet.octopart.com/MCL-S250GC-Multicomp-datasheet-12568751.pdf">datasheet.</a></li>
<li>Arrange components in accordance to some basic building blocks: <i>FTDI SERIAL HEADER</i>, <i>UPDI HEADER</i>, and <i>BUTTON</i>.</li>
<li>Choose the <strong>Place global label</strong> tool in order to assign some global "variables", such as GND or VCC, BTN or UPDI, etc. Separating blocks and wiring them (by pressing <kbd>W</kbd> to begin wire and <kbd>K</kbd> to end wire) with those global labels can make the schema more readable.</li>
<li>As common best practices, I added a pull-up resitors to the <i>BUTTON</i> block and wired a capacitor paralelly with the power supply.</li>
<li>The final step is to <strong>Generate Netlist</strong> which serves as a link between the schema and the PCB layout.</li>
</ul>
<p>Here you go all the components displayed and linked!</p>
<p></p>
<h6>Designing PCB with KiCad PCBnew</h6>
<p>When I <strong>Read Netlist</strong> in the PCB layout editor PCBnew of KiCAD, I immediately saw a horrible untangleable mess. The workflow used to save myself from that basically included 2 steps of Placement and Routing:</p>
<ul>
<li>Select pads from components and press <kbd>M</kbd> to move them or press <kbd>R</kbd> to rotate/flip them.</li>
<li>Re-arrange the components and give them rooms to breathe. By taking a look at the white lines, I can more or less imagined whether my components could be routed.</li>
...
<li>After struggling for a while, a good tip I learned was to connect all components with VCC pins first. The connections between GND pins will be done after the others, and they can joined to form an overall shape of the layout.</li>
</ul>
<p></p>
...
<p>Basically, no news is good news. However, I still have some issues which are indeed OK. For example, the ERC complained that I had some unused pins in the schema, and since I didn't create an edge cut in PCBnew, the DRC mistaked the outer traces as the edge of the design.</p>
